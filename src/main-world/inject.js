(function blocker() {
    ...
    main: {
      ...
      slimVideoMetadataSectionRenderer: {
        properties: {
          ...
          channelId:
            "contents.slimOwnerRenderer.navigationEndpoint.browseEndpoint.browseId",
    ...
    ytPlayer: {
      ...
      PLAYER_VARS: {
        properties: {
          ...
          title: [
            "raw_player_response.embedPreview.thumbnailPreviewRenderer.title",
          ],
          vidLength: [
  ...
  ObjectFilter.prototype.isDataEmpty = function () {
    ...
    if (
      !isNaN(storageData.filterData.vidLength[0]) ||
      !isNaN(storageData.filterData.vidLength[1])
    )
      return false;
  ...
  ObjectFilter.prototype.matchFilterData = function (filters, obj, objectType) {
    ...
    if (
      ...
      storageData.options.disable_on_history
    )
      return false;
    ...
    let doBlock = Object.keys(filters).some((h) => {
      ...
      if (
        ...
        (properties === undefined ||
          (properties.length === 0 && !jsFilterEnabled))
      )
        return false;
      ...
      if (
        ...
        !["/feed/history", "/feed/library", "/playlist"].includes(
      ...
      if (jsFilterEnabled) {
        if (h === "viewCount") {
          ...
          value.forEach((br) => {
            /* Channels */
            if (
              br.metadataBadgeRenderer.style === "BADGE_STYLE_TYPE_VERIFIED"
            ) {
              badges.push("verified");
            } else if (
              br.metadataBadgeRenderer.style ===
            ...
            /* Videos */
              ...
              br.metadataBadgeRenderer.style === "BADGE_STYLE_TYPE_MEMBERS_ONLY"
            ) {
              badges.push("members");
            }
          });
  ...
  ObjectFilter.prototype.isExtendedMatched = function (filteredObject, h) {
    ...
    if (
      storageData.options.shorts &&
      (h === "shortsLockupViewModel" ||
        h === "reelItemRenderer" ||
        h === "gridShelfViewModel")
    )
      return true;
    if (
      storageData.options.mixes &&
      (h === "radioRenderer" || h === "compactRadioRenderer")
    )
      return true;
    ...
    if (h === "commentViewModel") {
      if (
        this.blockedComments.includes(
          getObjectByPath(filteredObject, "commentId"),
        )
      ) {
        return true;
      }
  ...
  ObjectFilter.prototype.filter = function (obj = this.object) {
    ...
    if (this.contextMenus)
      !isMobileInterface ? addContextMenus(obj) : addContextMenusMobile(obj);
    return deletePrev;
  };
  ...
  function fixAutoPlayMobile() {
    ...
    let nextVideoRenderer = getObjectByPath(
      nextSection,
      "contents.videoWithContextRenderer",
    );
    ...
    autoplaySet.commandMetadata =
      nextVideoRenderer.navigationEndpoint.commandMetadata;
    autoplaySet.watchEndpoint =
      nextVideoRenderer.navigationEndpoint.watchEndpoint;
  }
  ...
  function redirectToNextMobile() {
    ...
    let nextAutoPlayObj = getObjectByPath(
      nextSection,
      "contents.videoWithContextRenderer",
    );
  ...
  function redirectToNext() {
    ...
    if (has.call(twoColumn, "conversationBar"))
  ...
  function playerMiscFilters() {
    ...
    if (storageData.options.disable_db_normalize === true) {
      const audioConfig = getObjectByPath(
        start_obj,
      ...
      const streamConfig = getObjectByPath(
        start_obj,
  ...
  function fetchFilter(url, resp) {
    ...
    if (["/youtubei/v1/search", "/youtubei/v1/browse"].includes(url.pathname)) {
      ...
      resp.forEach((o) => {
        if (
          o.responseType === "STREAMING_WATCH_RESPONSE_TYPE_PLAYER_RESPONSE"
        ) {
          ObjectFilter(o.playerResponse, filterRules.ytPlayer, [
            playerMiscFilters,
          ]);
        } else if (
          o.responseType === "STREAMING_WATCH_RESPONSE_TYPE_WATCH_NEXT_RESPONSE"
        ) {
  ...
  function spfFilter(url, resp) {
    ...
    ytDataArr.forEach((obj) => {
      if (has.call(obj, "player")) {
        try {
          const player_resp = getObjectByPath(
            obj.player,
  ...
  function findNextVideo(object) {
    ...
    const chipSection = secondaryResults.findIndex((x) =>
      has.call(x, "itemSectionRenderer"),
    );
    if (chipSection !== -1) {
    ...
    if (
      ...
      const blockCh = {
        menuServiceItemRenderer: {
          ...
          _bkMenuAction: "block_channel",
          _bkOriginalData: channelData,
      ...
      const blockVid = {
        menuServiceItemRenderer: {
          _bkOriginalAttr: attr,
          _bkMenuAction: "block_video",
          _bkOriginalData: videoData,
          ...
          buttons: [
            {
              slimMetadataButtonRenderer: {
                button: {
                  buttonRenderer: {
                    _bkOriginalData: videoData,
                    _bkOriginalAttr: "slimVideoMetadataSectionRenderer",
                    _bkMenuAction: "block_video",
                    style: "STYLE_DEFAULT",
                    size: "SIZE_DEFAULT",
                    isDisabled: false,
  ...
  function injectBlockMenuItems(
    ...
    if (isLockupViewModel) {
      return injectLockupViewModelButtons(
        items,
        hasChannel,
        hasVideo,
        currentObj,
      ...
      return injectStandardMenuButtons(
        items,
        hasChannel,
        hasVideo,
        storageData,
  ...
  function injectLockupViewModelButtons(
    items,
    hasChannel,
    hasVideo,
    currentObj,
    ...
    const cleanChannelContext = createCleanContext(
      items,
      storageData,
      true,
      currentObj,
    );
    const cleanVideoContext = createCleanContext(
      items,
      storageData,
      false,
      currentObj,
    ...
    const blockVideoItem = createLockupButtonItem(
      "Block Video",
      cleanVideoContext,
    );
  ...
  function createCleanContext(items, storageData, isChannel, currentObj) {
    let item;
    if (items.length === 8) item = isChannel ? items[6] : items[5];
    else item = isChannel ? items[5] : items[4];
  ...
  function injectStandardMenuButtons(items, hasChannel, hasVideo, storageData) {
    ...
    if (storageData.options.block_feedback) {
      for (const item of items) {
        ...
        const iconType = getObjectByPath(
          item,
          "menuServiceItemRenderer.icon.iconType",
        );
        if (iconType === "NOT_INTERESTED" && hasVideo) {
          blockVideoItem.menuServiceItemRenderer.serviceEndpoint =
            deepClone(endpoint);
        } else if (iconType === "REMOVE" && hasChannel) {
          blockChannelItem.menuServiceItemRenderer.serviceEndpoint =
            deepClone(endpoint);
        }
      }
    }
  ...
  function startHook() {
    if (window.location.pathname.startsWith("/embed/")) {
      const ytConfigPlayerConfig = getObjectByPath(
        window,
        "yt.config_.PLAYER_VARS",
      );
      if (
        typeof ytConfigPlayerConfig === "object" &&
        ytConfigPlayerConfig !== null
      ) {
        try {
  ...
  function storageReceived(data) {
    ...
    if (
      storageData.options.enable_javascript &&
      storageData.filterData.javascript
    ) {
      try {
        try {
    ...
    if (shouldStartHook && !window.bkDispatched) {
      startHook();
    }
  }
  ...
  function menuOnTapMobile(event) {
    ...
    if (window.bkReloadRequired) {
      window.bkExports.openToast(
        "Blocker was updated, this tab needs to be reloaded to use this function",
        5000,
      );
      return;
    }
    ...
    if (!data || !data._bkOriginalData) {
      return;
    }

    let type;
    switch (data._bkMenuAction) {
      case "block_channel": {
        type = "channelId";
        break;
      }
  ...
  function getBlockData(
    ...
    if (
      ...
      const ownerRenderer = document.getElementsByTagName(
      ...
      channelData = {
        id: parentData.blocker?.metadata?.channelId,
        text: parentData.blocker?.metadata?.channelName,
      };

      videoData = {
        id: parentData.blocker?.metadata?.videoId,
        text: parentData.blocker?.metadata?.videoName,
      };
  ...
  function menuOnTap(event) {
    ...
    const { parentDom, parentData } = getParentDomAndData(
      isDataFromRightHandSide,
      this,
    );
    ...
    (event) => {
      ...
      switch (event.data.type) {
        ...
        case "reloadRequired":
          ...
          openToast(
            "Blocker was updated, Please reload this tab to reactivate it",
            15000,
          );
          break;
        default:
