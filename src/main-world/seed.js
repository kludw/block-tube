(function () {
      ...
      return {
        get: function (target, key) {
          if (
            ...
            target[key] = new Proxy(
              target[key],
              getHandler(nextPath, nextPath.length == 0),
            );
        ...
        set: function (target, key, value) {
          if (enableHook && hookKeys.includes(key)) {
            function hook_() {
              if (window.bkDispatched) return value.apply(null, arguments);
              else
                window.addEventListener(
                  "blockerReady",
                  value.bind(null, arguments),
                );
            }
            target[key] = hook_;
          } else {
  ...
  const hooks = {
    menuOnTap(...args) {
      window.bkExports.menuOnTap.call(this, ...args);
    },
    menuOnTapMobile(...args) {
      window.bkExports.menuOnTapMobile.call(this, ...args);
    },
    genericHook(cb) {
      return function (...args) {
        if (window.bkDispatched) {
          ...
          window.addEventListener("blockerReady", () => {
            cb.call(this, ...args);
          });
        }
      };
  ...
  function setupPolymer(v) {
    return function (...args) {
      ...
      switch (args[0].is) {
        case "ytd-app":
          args[0].loadDesktopData_ = hooks.genericHook(
            args[0].loadDesktopData_,
          );
          break;
  ...
  function isUrlMatch(url) {
    ...
    return (
      spf_uris.some((uri) => uri === url.pathname) ||
      url.searchParams.has("pbj")
    );
  }
  ...
  function onPart(url, next) {
    return function (resp) {
      if (window.bkDispatched) {
        window.bkExports.spfFilter(url, resp);
        next(resp);
      } else
        window.addEventListener("blockerReady", () => {
          window.bkExports.spfFilter(url, resp);
  ...
  window.fetch = function (resource, init = undefined) {
    if (
      ...
      !fetch_uris.some((u) => resource.url.includes(u))
    ) {
      return org_fetch(resource, init);
    }

    return new Promise((resolve, reject) => {
      org_fetch(resource, (init = init))
        .then(function (resp) {
          ...
          resp
            ...
            .then(function (jsonResp) {
              if (window.bkDispatched) {
                window.bkExports.fetchFilter(url, jsonResp);
  ...
  if (window.location.pathname.startsWith("/embed/")) {
    ...
    Object.defineProperty(XMLHttpRequest.prototype, "response", {
      get: function () {
        ...
        let res = JSON.parse(
          XMLHttpRequestResponse.get.call(this).replace(")]}'", ""),
        );
        window.bkExports.fetchFilter(new URL(this.responseURL), res);
  ...
  Object.defineProperty(window, "writeEmbed", {
    ...
    set(v) {
      this.writeEmbed_ = () => {
        if (window.bkDispatched) v.apply(this);
        else window.addEventListener("blockerReady", v.bind(this));
      };
    },
  });
  ...
  Object.defineProperty(window, "loadInitialData", {
    ...
    set(v) {
      this.loadInitialData_ = (a1) => {
        if (window.bkDispatched) return v(a1);
        else window.addEventListener("blockerReady", v.bind(this, a1));
      };
    },
  });

  // player init has moved to window.yt.player.Application.create
  window.yt = createProxyHook("player.Application", [
    "create",
    "createAlternate",
  ]);
  ...
  if (!isMobileInterface) {
    ...
    Object.defineProperty(window.customElements, "define", {
      ...
      value: function (name, constructor) {
        if (
          name === "ytd-menu-service-item-renderer" ||
          name === "yt-list-item-view-model"
        ) {
        ...
        customElementsRegistryDefine.call(
